...
interface CostCodeRendererProps {
  ...
  onSelectionChange: (costCodeId: string, selection: Partial<CostCodeSelection>) => void;
...
export const CostCodeRenderer: React.FC<CostCodeRendererProps> = ({
  ...
  const renderCostCode = (costCode: CostCode) => {
    ...
    const textColor = QUESTION_TYPE_TEXT_COLORS[costCode.questionType as keyof typeof QUESTION_TYPE_TEXT_COLORS] || 'text-gray-600';
        ...
        <div key={costCode.id} className={`${bgColor} rounded-lg p-4 mb-4`}>
          ...
          <div className="flex items-center gap-4">
            <Input
              ...
              className="w-32"
            />
            <span className="text-sm text-gray-600">{costCode.unitType.replace('PER_', 'per ').toLowerCase()}</span>
          </div>
          {costCode.basePrice > 0 && (
    ...
    if (costCode.questionType === 'PURPLE') {
      return (
        <div key={costCode.id} className={`${bgColor} rounded-lg p-4 mb-4`}>
          <h3 className="font-semibold text-gray-900 mb-1">{costCode.name}</h3>
          <p className="text-sm text-gray-600">{costCode.description}</p>
          <p className={`text-xs ${textColor} mt-2`}>Calculated automatically</p>
        </div>
      );
    }
  ...
  return (
    <div className="space-y-8">
      {Object.entries(groupedCostCodes).map(([categoryName, categoryCostCodes]) => (
        <div key={categoryName} className="bg-white rounded-2xl p-8 shadow-sm">
