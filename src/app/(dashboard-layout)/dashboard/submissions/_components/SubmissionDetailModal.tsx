    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 overflow-y-auto">
      <div className="bg-white rounded-lg w-full max-w-4xl p-6 my-8 max-h-[90vh] overflow-y-auto">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="space-y-2 text-sm">
              <div>
                ...
                <span className="ml-2 font-medium">{submission.submissionNumber}</span>
              </div>
              <div>
                <span className="text-gray-600">Service:</span>
                <span className="ml-2 font-medium">{submission.service?.name}</span>
              </div>
              <div>
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="space-y-2 text-sm">
              <div>
                ...
                <span className="ml-2 font-medium">{submission.clientName}</span>
              </div>
              <div>
                <span className="text-gray-600">Email:</span>
                <span className="ml-2 font-medium">{submission.clientEmail}</span>
              </div>
              <div>
                <span className="text-gray-600">Phone:</span>
                <span className="ml-2 font-medium">{submission.clientPhone}</span>
              </div>
              <div>
                <span className="text-gray-600">Address:</span>
                <span className="ml-2 font-medium">{submission.projectAddress}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-blue-50 rounded-lg p-4 mb-6">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-medium">${submission.basePrice.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Additional Items:</span>
              <span className="font-medium">${submission.additionalItemsTotal.toLocaleString()}</span>
            </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {submission.items.map((item) => (
                    <tr key={item.id}>
                      <td className="px-4 py-3 text-sm text-gray-900">{item.costCode?.name || '-'}</td>
                      <td className="px-4 py-3 text-sm text-gray-600">{item.selectedOption?.optionName || item.userInputValue || '-'}</td>
                      <td className="px-4 py-3 text-sm text-gray-600">{item.quantity}</td>
                      <td className="px-4 py-3 text-sm text-gray-600 text-right">${item.unitPrice.toLocaleString()}</td>
                      <td className="px-4 py-3 text-sm font-medium text-gray-900 text-right">${(item.quantity * item.unitPrice).toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="mb-6">
            ...
            {submission.projectNotes && (
              <div className="bg-gray-50 rounded-lg p-4 mb-2">
                <p className="text-sm text-gray-700">{submission.projectNotes}</p>
              </div>
            )}
            {submission.additionalDetails && (
              <div className="bg-gray-50 rounded-lg p-4">
                <p className="text-sm text-gray-700">{submission.additionalDetails}</p>
              </div>
            )}
          </div>
        )}
        ...
        <div className="flex justify-end gap-3">
          {submission.pdfFile && (
            <a href={submission.pdfFile.url} target="_blank" rel="noopener noreferrer">
          ...
          <Button onClick={handleRegeneratePdf} variant="outline" disabled={regeneratePdfMutation.isPending}>
            <RefreshCw size={16} className="mr-2" />
            Regenerate PDF
          </Button>
          <Button onClick={onClose} className="bg-[#2d4a8f] hover:bg-[#243a73]">Close</Button>
        </div>
      </div>
    </div>
  );
